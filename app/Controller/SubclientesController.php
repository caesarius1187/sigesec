	...
	public function add() {
		if ($this->request->is('post')) {
			...
			if ($this->Subcliente->save($this->request->data)) {
				$this->Session->setFlash(__('El Subcliente ha sido Guardado.'));
				return $this->redirect(array('controller'=>'clientes','action' => 'view',$this->request->data['Subcliente']['cliente_id']));
			} else {
				$this->Session->setFlash(__('El Subcliente no ha sido Guardado. Por favor intente de nuevo mas tarde'));
			}
		}else{
			$this->Session->setFlash(__('El Subcliente no ha sido Guardado. Por favor intente de nuevo mas tarde'));
		}
